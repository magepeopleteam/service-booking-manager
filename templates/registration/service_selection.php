        ...
        <div class="_dShadow_7_mB_xs mpwpb_service_area">
			...
			<?php
				foreach ($all_services as $service_key => $service_item) {
					$category_name = array_key_exists('parent_cat', $service_item) ? ((is_numeric($service_item['parent_cat']) && $service_item['parent_cat'] >= 0) ? (int)$service_item['parent_cat'] + 1 : '') : '';
					$sub_category_name = array_key_exists('sub_cat', $service_item) ? ((is_numeric($service_item['sub_cat']) && $service_item['sub_cat'] >= 0) ? (int)$service_item['sub_cat'] + 1 : '') : '';
					...
					$service_id = $service_key + 1;
					//echo '<pre>'; print_r($sub_category_name); echo '</pre>';
					?>
                    <div class="mpwpb_service_item" data-price="<?php echo esc_attr($service_price); ?>" data-category="<?php echo esc_attr($category_name); ?>" data-sub-category="<?php echo esc_attr($sub_category_name); ?>" data-service="<?php echo esc_attr($service_id); ?>">
                        <div class="_dFlex">
