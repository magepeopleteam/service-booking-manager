<?php
	...
	if ( sizeof( $payment_system ) > 1 ) {
		...
		<div class="mpwpb_payment_system_area mT">
			<h3><?php esc_html_e( 'Select Payment System', 'bookingmaster' ); ?></h3>
				...
				<div class="allCenter">
					<?php if ( in_array( 'direct_order', $payment_system ) ) { ?>
						<div class="dLayout_xs min_200 mR" data-radio-check="direct_order" data-open-icon="fas fa-dot-circle" data-close-icon="far fa-circle">
							<h5 class="allCenter"><span data-icon class="far fa-circle mR_xs"></span><?php esc_html_e( 'Pay on service', 'bookingmaster' ); ?></h5>
						</div>
					<?php } ?>
					<?php if ( in_array( 'woocommerce', $payment_system ) ) { ?>
						<div class="dLayout_xs min_200" data-radio-check="woocommerce" data-open-icon="fas fa-dot-circle" data-close-icon="far fa-circle">
							<h5 class="allCenter"><span data-icon class="far fa-circle mR_xs"></span><?php esc_html_e( 'To pay', 'bookingmaster' ); ?></h5>
						</div>
					<?php } ?>
				</div>
			</div>
...
?>
	<div class="mpwpb_direct_order_info mT <?php echo esc_attr($current_payment_system=='direct_order'?'':'dNone'); ?>">
		<h3><?php esc_html_e( 'Billing Information', 'bookingmaster' ); ?></h3>
		<div class="divider"></div>
		<label>
			<span class="min_150"><?php esc_html_e( 'Billing Name : ', 'bookingmaster' ); ?></span>
			<input type="text" class="formControl mp_name_validation" name="mpwpb_bill_name" placeholder="<?php esc_attr_e( 'Billing Name Here....', 'bookingmaster' ); ?>"/>
		</label>
		<label class="mT_xs">
			<span class="min_150"><?php esc_html_e( 'Billing E-Mail : ', 'bookingmaster' ); ?></span>
			<input type="email" class="formControl" name="mpwpb_bill_email" placeholder="<?php esc_attr_e( 'Billing E-mail Here....', 'bookingmaster' ); ?>"/>
		</label>
	</div>
	...
	<div class="mpwpb_book_now_area">
		...
		<div class="justifyCenter mT_xs">
			<button class="_mpBtn_mT_xs_radius mActive mpwpb_book_now" data-submit-path="<?php echo esc_attr( get_home_url() . '/mpwpb-order-details/' ); ?>" type="button" data-alert="<?php esc_attr_e( 'Please select Payment Method', 'bookingmaster' ); ?>">
				<span class="fas fa-cart-plus mR_xs"></span>
				<?php esc_html_e( 'Proceed Order', 'bookingmaster' ); ?>
			</button>
