...
*/
	...
	if ( ! class_exists( 'MPWPB_Custom_Slider' ) ) {
		class MPWPB_Custom_Slider {
			public function __construct() {
				add_action( 'add_mpwpb_custom_slider', array( $this, 'super_slider' ), 10, 2 );
				add_action( 'add_mpwpb_custom_slider_only', array( $this, 'super_slider_only' ) );
				add_action( 'add_mpwpb_custom_slider_icon_indicator', array( $this, 'icon_indicator' ) );
			}
			public function super_slider( $post_id = '', $meta_key = '' ) {
				$type      = MPWPB_Global_Function::get_slider_settings( 'slider_type', 'slider' );
			...
			public function slider( $post_id, $image_ids ) {
				if ( is_array( $image_ids ) && sizeof( $image_ids ) > 0 ) {
					...
					<div class="superSlider placeholder_area fdColumn">
						...
						<?php
							$slider_indicator = MPWPB_Global_Function::get_slider_settings( 'indicator_visible', 'on' );
							$icon             = MPWPB_Global_Function::get_slider_settings( 'indicator_type', 'icon' );
			...
			public function post_thumbnail( $image_id = '' ) {
				$thumbnail = MPWPB_Global_Function::get_image_url( '', $image_id );
				if ( $thumbnail ) {
					?>
					...
					<div class="sliderAllItem">
						<?php
							...
							foreach ( $image_ids as $id ) {
								$image_url = MPWPB_Global_Function::get_image_url( '', $id );
								?>
			...
			public function slider_showcase( $image_ids ) {
				$showcase = MPWPB_Global_Function::get_slider_settings( 'showcase_visible', 'on' );
				...
				if ( is_array( $image_ids ) && sizeof( $image_ids ) > 0 ) {
					...
					<div class="slideIndicator">
						<?php
							$count = 1;
							foreach ( $image_ids as $id ) {
								$image_url = MPWPB_Global_Function::get_image_url( '', $id, array( 150, 100 ) );
								?>
					...
					<div class="sliderPopup" data-popup="superSlider">
						<div class="superSlider">
							...
							<div class="popupFooter">
								<?php
									$indicator = MPWPB_Global_Function::get_slider_settings( 'popup_image_indicator', 'on' );
									if ( $indicator == 'on' ) {
										$this->image_indicator( $image_ids );
									}
								?>
			...
			public function get_slider_ids( $post_id, $key ) {
				...
				$image_ids = MPWPB_Global_Function::get_post_info( $post_id, $key, array() );
				if ( $thumb_id ) {
