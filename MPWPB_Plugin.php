	...
	if ( ! class_exists( 'MPWPB_Plugin' ) ) {
		class MPWPB_Plugin {
			...
			public function on_activation_page_create() {
				if ( ! $this->get_page_by_slug( 'mpwpb-order-details' ) ) {
					$add_page = array(
						...
						'post_title'   => esc_html__( 'Order Details', 'bookingplus' ),
			...
			public function woocommerce_not_active() {
				...
				printf( '<div class="error" style="background:red; color:#fff;"><p>%s</p></div>', __( 'You Must Install WooCommerce Plugin before activating Bookingplus, Because It is dependent on Woocommerce Plugin. <a class="btn button" href=' . $wc_install_url . '>Click Here to Install</a>' ) );
			}
		}
		new MPWPB_Plugin();
	}